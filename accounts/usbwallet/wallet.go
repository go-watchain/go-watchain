...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the watchain
...
type wallet struct {
	...
	deriveChain    watereum.ChainStateReader // Blockchain state reader to discover used account with
...
// whwater they are still present or if they malfunctioned.
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived watchain account
			...
			// If the next account is empty, stop self-derivation, but add it nonwateless
...
// Contains implements accounts.Wallet, returning whwater a particular account is
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain watereum.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
