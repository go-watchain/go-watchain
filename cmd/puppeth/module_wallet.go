...
FROM puppwat/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gwat --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'gwat --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --watstats \'{{.watstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*"' >> wallet.sh

RUN \
	sed -i 's/PuppwatNetworkID/{{.NetworkID}}/g' dist/js/waterwallet-master.js && \
	sed -i 's/PuppwatNetwork/{{.Network}}/g'     dist/js/waterwallet-master.js && \
	sed -i 's/PuppwatDenom/{{.Denom}}/g'         dist/js/waterwallet-master.js && \
	sed -i 's/PuppwatHost/{{.Host}}/g'           dist/js/waterwallet-master.js && \
	sed -i 's/PuppwatRPCPort/{{.RPCPort}}/g'     dist/js/waterwallet-master.js
...
// checkWallet does a health-check against web wallet server to verify whwater
// it's running, and if yes, whwater it's responsive.
