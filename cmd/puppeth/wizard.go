...
// config contains all the configurations needed by puppwat that should be saved
// between sessions.
type config struct {
	path      string   // File containing the configuration values
	bootnodes []string // Bootnodes to always connect to by all nodes
	watstats  string   // watstats settings to cache for node deploys
...
func (c config) flush() {
	...
	if err := ioutil.WriteFile(c.path, out, 0644); err != nil {
		log.Warn("Failed to save puppwat configs", "file", c.path, "err", err)
	}
}
...
type wizard struct {
	...
	services map[string][]string   // watchain services known to be running on servers
...
// readDefaulwatring reads a single line from stdin, trimming if from spaces. If
// an empty line is entered, the default value is returned.
func (w *wizard) readDefaulwatring(def string) string {
	fmt.Printf("> ")
	text, err := w.in.ReadString('\n')
	if err != nil {
		log.Crit("Failed to read user input", "err", err)
...
func (w *wizard) readDefaultBigInt(def *big.Int) *big.Int {
	for {
		...
		val, ok := new(big.Int).Sewatring(text, 0)
		if !ok {
...
func (w *wizard) readAddress() *common.Address {
	for {
		...
		bigaddr, _ := new(big.Int).Sewatring(text, 16)
...
// converts it to an watchain address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by watstats, which compares IPs textually, not by value.
