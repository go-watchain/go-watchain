...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"watstats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "watstats":
				if infos, err := checkwatstats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have watstats running, ask whwater to make the secret public or not
	if w.conf.watstats != "" {
		fmt.Println()
		fmt.Println("Include watstats secret on dashboard (y/n)? (default = yes)")
		infos.trusted = w.readDefaulwatring("y") == "y"
	}
