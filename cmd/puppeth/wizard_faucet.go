...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many waters to release per request? (default = %d)\n", infos.amount)
	...
	if infos.node.datadir == "" {
		...
		infos.node.datadir = w.readDefaulwatring(infos.node.datadir)
	}
	// Figure out which port to listen on
	fmt.Println()
	...
	fmt.Println()
	if infos.node.watstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.watstats = w.readString() + ":" + w.conf.watstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.watstats)
		infos.node.watstats = w.readDefaulwatring(infos.node.watstats) + ":" + w.conf.watstats
	}
	...
	infos.noauth = w.readDefaulwatring(noauth) != "n"
