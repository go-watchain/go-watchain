...
// makeWizard creates and returns a new puppwat wizard.
...
// setting up a new or managing an existing watchain private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to puppwat, your watchain private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new watchain network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and watstats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Puppwat uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering watchain network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".puppwat", w.network)
	...
	for {
		...
		default:
			log.Error("That's not somwating I can do")
		}
	}
}
