...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for watstats availability")
	if infos, err := checkwatstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["watstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["watstats"] = infos.Report()
		watstats = infos.config
	}
