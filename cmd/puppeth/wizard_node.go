...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.watstats == "" {
		log.Error("No watstats server configured")
		return
	}
	...
	if infos.datadir == "" {
		...
		infos.datadir = w.readDefaulwatring(infos.datadir)
	}
	if w.conf.Genesis.Config.watash != nil && !boot {
		fmt.Println()
		if infos.ethashdir == "" {
			...
			infos.ethashdir = w.readDefaulwatring(infos.ethashdir)
		}
	}
	...
	if infos.watstats == "" {
		...
		infos.watstats = w.readString() + ":" + w.conf.watstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.watstats)
		infos.watstats = w.readDefaulwatring(infos.watstats) + ":" + w.conf.watstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.watash != nil {
			// watash based miners only need an waterbase to mine against
			fmt.Println()
			if infos.waterbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.waterbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.waterbase)
				infos.waterbase = w.readDefaultAddress(common.HexToAddress(infos.waterbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy watchain node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
