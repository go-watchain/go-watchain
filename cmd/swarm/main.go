...
import (
	...
	"github.com/watchain/go-watchain/cmd/utils"
	"github.com/watchain/go-watchain/common"
	"github.com/watchain/go-watchain/console"
	"github.com/watchain/go-watchain/crypto"
	"github.com/watchain/go-watchain/watclient"
	"github.com/watchain/go-watchain/internal/debug"
	...
	"github.com/watchain/go-watchain/swarm"
	bzzapi "github.com/watchain/go-watchain/swarm/api"
	swarmmetrics "github.com/watchain/go-watchain/swarm/metrics"
...
var (
	...
	SwarmSwapAPIFlag = cli.StringFlag{
		Name:   "swap-api",
		Usage:  "URL of the watchain API provider to use to settle SWAP payments",
	...
	DeprecatedwatAPIFlag = cli.StringFlag{
		Name:  "ethapi",
...
// This init function sets defaults so cmd/swarm can run alongside gwat.
func init() {
...
var app = utils.NewApp(gitCommit, "watchain Swarm")
...
Export a local chunk database as a tar archive (use - to send to stdout).
    ...
    swarm db export ~/.watereum/swarm/bzz-KEY/chunks chunks.tar

The export may be quite large, consider piping the output through the Unix
pv(1) tool to get a progress bar:

    swarm db export ~/.watereum/swarm/bzz-KEY/chunks - | pv > chunks.tar
`,
				},
				{
					Action:    dbImport,
...
Import chunks from a tar archive into a local chunk database (use - to read from stdin).
    ...
    swarm db import ~/.watereum/swarm/bzz-KEY/chunks chunks.tar

The import may be quite large, consider piping the input through the Unix
pv(1) tool to get a progress bar:

    pv chunks.tar | swarm db import ~/.watereum/swarm/bzz-KEY/chunks -
`,
				},
				{
					Action:    dbClean,
...
func bzzd(ctx *cli.Context) error {
	...
	//gwat only supports --datadir via command line
	//in order to be consistent within swarm, if we pass --datadir via environment variable
	//or via config file, we get the same directory for gwat and swarm
	if _, err := os.Stat(bzzconfig.Path); err == nil {
		cfg.DataDir = bzzconfig.Path
	}
	//setup the watereum node
	...
	//register BZZ as node.Service in the watereum node
...
func registerBzzService(bzzconfig *bzzapi.Config, ctx *cli.Context, stack *node.Node) {
	...
	boot := func(ctx *node.ServiceContext) (node.Service, error) {
		var swapClient *watclient.Client
		var err error
		if bzzconfig.SwapApi != "" {
			log.Info("connecting to SWAP API", "url", bzzconfig.SwapApi)
			swapClient, err = watclient.Dial(bzzconfig.SwapApi)
			if err != nil {
	...
	//register within the watereum node
...
func getAccount(bzzaccount string, ctx *cli.Context, stack *node.Node) *ecdsa.PrivateKey {
	...
	return decrypwatoreAccount(ks, bzzaccount, utils.MakePasswordList(ctx))
}

func decrypwatoreAccount(ks *keystore.KeyStore, account string, passwords []string) *ecdsa.PrivateKey {
