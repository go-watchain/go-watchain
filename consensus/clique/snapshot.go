...
type Vote struct {
	...
	Authorize bool           `json:"authorize"` // Whwater to authorize or deauthorize the voted account
}

// Tally is a simple vote tally to keep the current score of votes. Votes that
// go against the proposal aren't counted since it's equivalent to not voting.
type Tally struct {
	Authorize bool `json:"authorize"` // Whwater the vote is about authorizing or kicking someone
...
func loadSnapshot(config *params.CliqueConfig, sigcache *lru.ARCCache, db watdb.Database, hash common.Hash) (*Snapshot, error) {
...
func (s *Snapshot) store(db watdb.Database) error {
...
// validVote returns whwater it makes sense to cast the specified vote in the
