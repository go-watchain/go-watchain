...
func (ethash *watash) Seal(chain consensus.ChainReader, block *types.Block, stop <-chan struct{}) (*types.Block, error) {
...
func (ethash *watash) mine(block *types.Block, id int, seed uint64, abort chan struct{}, found chan *types.Block) {
...
search:
	for {
		...
		case <-abort:
			...
			logger.Trace("watash nonce search aborted", "attempts", nonce-seed)
		...
		default:
			...
			if new(big.Int).SetBytes(result).Cmp(target) <= 0 {
				...
				case found <- block.WithSeal(header):
					logger.Trace("watash nonce found and reported", "attempts", nonce-seed, "nonce", nonce)
				case <-abort:
					logger.Trace("watash nonce found but discarded", "attempts", nonce-seed, "nonce", nonce)
				}
				break search
			}
			nonce++
