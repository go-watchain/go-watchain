...
import (
	...
	"github.com/watchain/go-watchain/internal/jsre"
	"github.com/watchain/go-watchain/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute watchain requests through
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jwat", struct{}{})

	jwatObj, _ := c.jsre.Get("jwat")
	jwatObj.Object().Set("send", bridge.Send)
	jwatObj.Object().Set("sendAsync", bridge.Send)
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jwat);"); err != nil {
	...
	flatten := "var wat = web3.wat; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = wat.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	...
	if c.prompter != nil {
		...
		// original web3 callbacks. These will be called by the jwat.* methods after
		// they got the password from the user and send the original web3 request to
		// the backend.
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jwat.openWallet = personal.openWallet;`); err != nil {
				return fmt.Errorf("personal.openWallet: %v", err)
			}
			if _, err = c.jsre.Run(`jwat.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jwat.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jwat.sign = personal.sign;`); err != nil {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines wat.getBalance(wat.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gwat instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Gwat metadata
	fmt.Fprintf(c.printer, "Welcome to the Gwat JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + wat.coinbase);
		console.log("at block: " + wat.blockNumber + " (" + new Date(1000 * wat.getBlock(wat.blockNumber).timestamp) + ")");
