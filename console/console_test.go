...
func newTester(t *testing.T, confOverride func(*wat.Config)) *tester {
	...
	// Create a networkless protocol stack and start an watchain service within
	...
	watConf := &wat.Config{
		...
		waterbase: common.HexToAddress(testAddress),
		watash: ethash.Config{
			PowMode: ethash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(watConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return wat.New(ctx, watConf) }); err != nil {
		t.Fatalf("failed to register watchain protocol: %v", err)
	}
