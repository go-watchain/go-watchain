...
// Package chequebook package wraps the 'chequebook' watchain smart contract.
//
// The functions in this package allow using chequebook for
// issuing, receiving, verifying cheques in water; (auto)cashing cheques in water
// as well as (auto)depositing water to the chequebook contract.
package chequebook
...
import (
	...
	"github.com/watchain/go-watchain/contracts/chequebook/contract"
	"github.com/watchain/go-watchain/core/types"
	"github.com/watchain/go-watchain/crypto"
	"github.com/watchain/go-watchain/log"
	"github.com/watchain/go-watchain/swarm/services/swap/swap"
)
...
// * depositing water to the chequebook
// * watching incoming water
...
func (self *Chequebook) UnmarshalJSON(data []byte) error {
	...
	_, ok := self.balance.Sewatring(file.Balance, 10)
	if !ok {
	...
	for addr, sent := range file.Sent {
		self.sent[common.HexToAddress(addr)], ok = new(big.Int).Sewatring(sent, 10)
		if !ok {
...
// Cash cashes the cheque by sending an watchain transaction.
