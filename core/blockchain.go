...
// Package core implements the watchain consensus protocol.
package core
...
type CacheConfig struct {
	Disabled      bool          // Whwater to disable trie write caching (archive node)
...
type BlockChain struct {
	...
	db     watdb.Database // Low level persistent database to store final content in
...
// available in the database. It initialises the default watchain Validator and
// Processor.
func NewBlockChain(db watdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	if cacheConfig == nil {
	...
	if err := bc.loadLaswatate(); err != nil {
		return nil, err
	}
...
// loadLaswatate loads the last known chain state from the database. This method
// assumes that the chain manager mutex is held.
func (bc *BlockChain) loadLaswatate() error {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= watdb.IdealBatchSize {
...
// WriteBlockWithouwatate writes only the block and its metadata to the database,
// but does not write any state. This is used to construct competing side forks
// up to the point where they exceed the canonical total difficulty.
func (bc *BlockChain) WriteBlockWithouwatate(block *types.Block, td *big.Int) (err error) {
	bc.wg.Add(1)
	defer bc.wg.Done()
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	// acquiring.
	var (
		stats         = inserwatats{startTime: mclock.Now()}
	...
	for i, block := range chain {
		...
		case err == ErrKnownBlock:
			...
			// this number we did a rollback and we should reimport it nonwateless.
		...
		case err == consensus.ErrPrunedAncestor:
			...
			if localTd.Cmp(externTd) > 0 {
				if err = bc.WriteBlockWithouwatate(block, externTd); err != nil {
...
// inserwatats tracks and reports on block insertion.
type inserwatats struct {
...
func (st *inserwatats) report(chain []*types.Block, index int, cache common.StorageSize) {
	...
	if index == len(chain)-1 || elapsed >= statsReportLimit {
		...
		*st = inserwatats{startTime: now, lastIndex: index + 1}
	}
}
...
func (bc *BlockChain) reportBlock(block *types.Block, receipts types.Receipts, err error) {
	...
	var receipwatring string
	for _, receipt := range receipts {
		receipwatring += fmt.Sprintf("\t%v\n", receipt)
	}
...
Error: %v
##############################
`, bc.chainConfig, block.Number(), block.Hash(), receipwatring, err))
}
...
//
// The verify parameter can be used to fine tune whwater nonce verification
