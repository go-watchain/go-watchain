...
var (
	...
	configPrefix   = []byte("watereum-config-") // config prefix for the db
...
func WriteCanonicalHash(db watdb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db watdb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db watdb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db watdb.Putter, hash common.Hash) error {
...
func WriteTrieSyncProgress(db watdb.Putter, count uint64) error {
...
func WriteHeader(db watdb.Putter, header *types.Header) error {
...
func WriteBody(db watdb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db watdb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db watdb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db watdb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db watdb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db watdb.Putter, block *types.Block) error {
...
func WriteBloomBits(db watdb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db watdb.Database) watdb.Database {
	return watdb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db watdb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db watdb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db watdb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
