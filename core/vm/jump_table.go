...
type operation struct {
	...
	halts   bool // indicates whwater the operation should halt further execution
	jumps   bool // indicates whwater the program counter should not increment
	writes  bool // determines whwater this a state modifying operation
	valid   bool // indication whwater the retrieved operation is valid and known
	reverts bool // determines whwater the operation reverts state (implicitly halts)
	returns bool // determines whwater the operations sets the return data content
}
...
func NewConstantinopleInstructionSet() [256]operation {
	...
	instructionSet[SHL] = operation{
		execute:       opSHL,
		gasCost:       constGasFunc(GasFasteswatep),
...
func NewFrontierInstructionSet() [256]operation {
	return [256]operation{
		...
		MUL: {
			execute:       opMul,
			gasCost:       constGasFunc(GasFaswatep),
			validateStack: makeStackFunc(2, 1),
			valid:         true,
		},
		SUB: {
		...
		BLOCKHASH: {
			execute:       opBlockhash,
			gasCost:       constGasFunc(GasExwatep),
			validateStack: makeStackFunc(1, 1),
			valid:         true,
		},
		COINBASE: {
