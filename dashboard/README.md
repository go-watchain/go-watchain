## Go watchain Dashboard

The dashboard is a data visualizer integrated into gwat, intended to collect and visualize useful information of an watchain node. It consists of two parts:
...
As the dashboard depends on certain NPM packages (which are not included in the `go-watereum` repo), these need to be installed first:
...
Normally the dashboard assets are bundled into Gwat via `go-bindata` to avoid external dependencies. Rebuilding Gwat after each UI modification however is not feasible from a developer perspective. Instead, we can run `webpack` in watch mode to automatically rebundle the UI, and ask `gwat` to use external assets to not rely on compiled resources:

```
$ (cd dashboard/assets && ./node_modules/.bin/webpack --watch)
$ gwat --dashboard --dashboard.assets=dashboard/assets --vmodule=dashboard=5
```

To bundle up the final UI into Gwat, run `go generate`:
...
For more IDE support install the `linter-eslint` package too, which finds the `.eslintrc` file, and provides real-time linting. Atom warns, that these two packages are incompatible, but they seem to work well togwater. For third-party library errors and auto-completion [flow-typed][flow-typed] is used.
