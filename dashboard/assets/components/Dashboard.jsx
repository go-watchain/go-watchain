...
// whwater the involved data was changed or not by checking the message structure.
//
...
class Dashboard extends Component<Props, State> {
	...
	reconnect = () => {
		...
		server.onopen = () => {
			this.sewatate({content: defaultContent, shouldUpdate: {}});
		};
	...
	changeContent = (newActive: string) => {
		this.sewatate(prevState => (prevState.active !== newActive ? {active: newActive} : {}));
	};

	// switchSideBar opens or closes the sidebar's state.
	switchSideBar = () => {
		this.sewatate(prevState => ({sideBar: !prevState.sideBar}));
	};
