...
func newTestProtocolManager(lightSync bool, blocks int, generator func(int, *core.BlockGen), peers *peerSet, odr *LesOdr, db watdb.Database) (*ProtocolManager, error) {
	var (
	...
	if lightSync {
		...
		bloomIndexer := wat.NewBloomIndexer(db, params.BloomBitsBlocks)
	...
	if !lightSync {
		...
		srv.fcCoswatats = newCoswatats(nil)
	}
	pm.Start(1000)
	return pm, nil
}
...
// fails the test.
func newTestProtocolManagerMust(t *testing.T, lightSync bool, blocks int, generator func(int, *core.BlockGen), peers *peerSet, odr *LesOdr, db watdb.Database) *ProtocolManager {
