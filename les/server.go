...
import (
	...
	"github.com/watchain/go-watchain/les/flowcontrol"
...
func NewLesServer(wat *wat.watchain, config *wat.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(wat.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, wat.EventMux(), wat.Engine(), newPeerSet(), wat.BlockChain(), wat.TxPool(), wat.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(wat.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(wat.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(wat.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(wat.BlockChain())
	pm.server = srv
	...
	srv.fcCoswatats = newCoswatats(wat.ChainDb())
	return srv, nil
}
...
type requestCoswatats struct {
	lock  sync.RWMutex
	db    watdb.Database
	stats map[uint64]*linReg
}

type requestCoswatatsRlp []struct {
	MsgCode uint64
	Data    []byte
}

var rcStatsKey = []byte("_requestCoswatats")

func newCoswatats(db watdb.Database) *requestCoswatats {
	...
	if db != nil {
		...
		var statsRlp requestCoswatatsRlp
		if err == nil {
	...
	return &requestCoswatats{
		db:    db,
		stats: stats,
	}
}

func (s *requestCoswatats) store() {
	s.lock.Lock()
	defer s.lock.Unlock()

	statsRlp := make(requestCoswatatsRlp, len(reqList))
...
func (s *requestCoswatats) getCurrentList() RequestCostList {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *requestCoswatats) update(msgCode, reqCnt, cost uint64) {
	s.lock.Lock()
	defer s.lock.Unlock()
