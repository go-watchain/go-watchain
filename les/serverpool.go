...
const (
	...
	// iniwatatsWeight is used to initialize previously unknown peers with good
...
type serverPool struct {
	...
	adjuswatats          chan poolStatAdjust
...
func newServerPool(db watdb.Database, quit chan struct{}, wg *sync.WaitGroup) *serverPool {
	pool := &serverPool{
		db:           db,
		quit:         quit,
		wg:           wg,
		...
		adjuswatats:  make(chan poolStatAdjust, 100),
...
func (pool *serverPool) disconnect(entry *poolEntry) {
	...
	if entry.state == psRegistered {
		...
		if stopped {
			entry.connecwatats.add(1, connAdjust)
		} else {
			entry.connecwatats.add(connAdjust, 1)
		}
	}
...
func (pool *serverPool) adjustBlockDelay(entry *poolEntry, time time.Duration) {
	...
	pool.adjuswatats <- poolStatAdjust{pseBlockDelay, entry, time}
}
...
func (pool *serverPool) adjustResponseTime(entry *poolEntry, time time.Duration, timeout bool) {
	...
	if timeout {
		pool.adjuswatats <- poolStatAdjust{pseResponseTimeout, entry, time}
	} else {
		pool.adjuswatats <- poolStatAdjust{pseResponseTime, entry, time}
	}
}
	...
	for {
		...
		case adj := <-pool.adjuswatats:
			...
			case pseResponseTime:
				...
				adj.entry.timeouwatats.add(0, 1)
			case pseResponseTimeout:
				adj.entry.timeouwatats.add(1, 1)
			}
			pool.lock.Unlock()
...
func (pool *serverPool) findOrNewNode(id discover.NodeID, ip net.IP, port uint16) *poolEntry {
	...
	if entry == nil {
		...
		entry.connecwatats.add(1, iniwatatsWeight)
		entry.delayStats.add(0, iniwatatsWeight)
		entry.responseStats.add(0, iniwatatsWeight)
		entry.timeouwatats.add(0, iniwatatsWeight)
	}
...
func (pool *serverPool) loadNodes() {
	...
	for _, e := range list {
		log.Debug("Loaded server stats", "id", e.id, "fails", e.lastConnected.fails,
			"conn", fmt.Sprintf("%v/%v", e.connecwatats.avg, e.connecwatats.weight),
			"delay", fmt.Sprintf("%v/%v", time.Duration(e.delayStats.avg), e.delayStats.weight),
			"response", fmt.Sprintf("%v/%v", time.Duration(e.responseStats.avg), e.responseStats.weight),
			"timeout", fmt.Sprintf("%v/%v", e.timeouwatats.avg, e.timeouwatats.weight))
		pool.entries[e.id] = e
...
type poolEntry struct {
	...
	connecwatats, delayStats    poolStats
	responseStats, timeouwatats poolStats
	state                       int
...
func (e *poolEntry) EncodeRLP(w io.Writer) error {
	return rlp.Encode(w, []interface{}{e.id, e.lastConnected.ip, e.lastConnected.port, e.lastConnected.fails, &e.connecwatats, &e.delayStats, &e.responseStats, &e.timeouwatats})
}
...
func (e *poolEntry) DecodeRLP(s *rlp.Stream) error {
	var entry struct {
		...
		Port                       uint16
		Fails                      uint
		CStat, DStat, RStat, watat poolStats
	}
	...
	e.connecwatats = entry.CStat
	e.delayStats = entry.DStat
	e.responseStats = entry.RStat
	e.timeouwatats = entry.watat
...
func (e *knownEntry) Weight() int64 {
	...
	return int64(1000000000 * e.connecwatats.recentAvg() * math.Exp(-float64(e.lastConnected.fails)*failDropLn-e.responseStats.recentAvg()/float64(responseScoreTC)-e.delayStats.recentAvg()/float64(delayScoreTC)) * math.Pow(1-e.timeouwatats.recentAvg(), timeoutPow))
}
