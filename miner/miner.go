...
// Package miner implements watchain block creation and mining.
package miner
...
type Miner struct {
	...
	canStart    int32 // can start indicates whwater we can start the mining operation
	shouldStart int32 // should start indicates whwater we should start after sync
}

func New(wat Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine) *Miner {
	miner := &Miner{
		wat:      wat,
		mux:      mux,
		engine:   engine,
		worker:   newWorker(config, engine, common.Address{}, wat, mux),
		canStart: 1,
	}
	miner.Register(NewCpuAgent(wat.BlockChain(), engine))
	go miner.update()
...
func (self *Miner) Start(coinbase common.Address) {
	atomic.StoreInt32(&self.shouldStart, 1)
	self.worker.setwaterbase(coinbase)
...
func (self *Miner) Setwaterbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setwaterbase(addr)
}
