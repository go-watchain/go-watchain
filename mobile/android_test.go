...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gwat.LightScryptN, Gwat.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.watereum", new NodeConfig());
			node.start();
			...
			watchainClient ec = node.getwatchainClient();
	...
	// signatures too. Regression test for go-watereum issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogwater
	...
	workspace, err := ioutil.TempDir("", "gwat-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/watereum/gwattest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gwat and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.watereum", "github.com/watchain/go-watchain/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gwat.aar"), "gwat.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "watereum", "gwattest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
