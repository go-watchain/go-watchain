...
func (n *Node) OpenDatabase(name string, cache, handles int) (watdb.Database, error) {
	if n.config.DataDir == "" {
		return watdb.NewMemDatabase()
	}
	return watdb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
