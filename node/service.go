...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (watdb.Database, error) {
	if ctx.config.DataDir == "" {
		return watdb.NewMemDatabase()
	}
	db, err := watdb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
