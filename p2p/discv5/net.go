...
import (
	...
	"github.com/watchain/go-watchain/crypto/sha3"
...
func (net *Network) SetFallbackNodes(nodes []*Node) error {
	...
	for _, n := range nodes {
		if err := n.validateComplete(); err != nil {
			return fmt.Errorf("bad boowatrap/fallback node %q (%v)", n, err)
		}
...
func (net *Network) loop() {
	...
	resetNextTicket := func() {
		ticket, timeout := net.tickewatore.nextFilteredTicket()
...
loop:
	for {
		...
		case req := <-net.topicRegisterReq:
			...
			if !req.add {
				net.tickewatore.removeRegisterTopic(req.topic)
				continue
			}
			net.tickewatore.addTopic(req.topic, true)
			...
			if topicRegisterLookupTarget.target == (common.Hash{}) {
				...
				target, delay := net.tickewatore.nextRegisterLookup()
		...
		case nodes := <-topicRegisterLookupDone:
			...
			net.tickewatore.registerLookupDone(topicRegisterLookupTarget, nodes, func(n *Node) []byte {
				net.ping(n, n.addr())
				return n.pingEcho
			})
			target, delay := net.tickewatore.nextRegisterLookup()
		...
		case <-nextRegisterTime:
			...
			net.tickewatore.ticketRegistered(*nextTicket)
			...
			if refreshDone == nil {
				...
				if ok {
					if req.delay == time.Duration(0) {
						...
						net.tickewatore.removeSearchTopic(req.topic)
					} else {
				...
				if req.delay != time.Duration(0) {
					...
					searchInfo[req.topic] = info
					net.tickewatore.addSearchTopic(req.topic, req.found)
		...
		case topic := <-topicSearch:
			if activeSearchCount < maxSearchCount {
				activeSearchCount++
				target := net.tickewatore.nextSearchLookup(topic)
				go func() {
		...
		case res := <-topicSearchLookupDone:
			...
			if lookupChn := searchInfo[res.target.topic].lookupChn; lookupChn != nil {
				lookupChn <- net.tickewatore.radius[res.target.topic].converged
			}
			net.tickewatore.searchLookupDone(res.target, res.nodes, func(n *Node, topic Topic) []byte {
		...
		case <-statsDump.C:
			...
			/*r, ok := net.tickewatore.radius[testTopic]
			if !ok {
				fmt.Printf("(%x) no radius @ %v\n", net.tab.self.ID[:8], time.Now())
			} else {
				topics := len(net.tickewatore.tickets)
				tickets := len(net.tickewatore.nodes)
				rad := r.radius / (maxRadius/10000+1)
				fmt.Printf("(%x) topics:%d radius:%d tickets:%d @ %v\n", net.tab.self.ID[:8], topics, rad, tickets, time.Now())
			}*/

			tm := mclock.Now()
			for topic, r := range net.tickewatore.radius {
				if printTestImgLogs {
...
func (net *Network) refresh(done chan<- struct{}) {
	...
	for _, n := range seeds {
		...
		// Force-add the seed node so Lookup does somwating.
...
func (net *Network) ping(n *Node, addr *net.UDPAddr) {
	...
	n.pingTopics = net.tickewatore.regTopicSet()
...
func (net *Network) handleKnownPong(n *Node, pkt *ingressPacket) error {
	...
	if err == nil {
		...
		net.tickewatore.addTicket(now, pkt.data.(*pong).ReplyTok, ticket)
	} else {
...
func (net *Network) handleQueryEvent(n *Node, ev nodeEvent, pkt *ingressPacket) (*nodeState, error) {
	...
	case topicQueryPacket:
		...
		if _, ok := net.tickewatore.tickets[topic]; ok {
	...
	case topicNodesPacket:
		...
		if net.tickewatore.gotTopicNodes(n, p.Echo, p.Nodes) {
			n.queryTimeouts++
			if n.queryTimeouts > maxFindnodeFailures && n.state == known {
				return contested, errors.New("too many timeouts")
			}
