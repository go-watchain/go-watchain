...
func newTickewatore() *tickewatore {
	return &tickewatore{
...
func (s *tickewatore) addTopic(topic Topic, register bool) {
...
func (s *tickewatore) addSearchTopic(t Topic, foundChn chan<- *Node) {
	s.addTopic(t, false)
...
func (s *tickewatore) removeSearchTopic(t Topic) {
...
func (s *tickewatore) removeRegisterTopic(topic Topic) {
...
func (s *tickewatore) regTopicSet() []Topic {
...
func (s *tickewatore) nextRegisterLookup() (lookupInfo, time.Duration) {
...
func (s *tickewatore) nextSearchLookup(topic Topic) lookupInfo {
	tr := s.radius[topic]
...
func (s *tickewatore) ticketsInWindow(topic Topic) []ticketRef {
...
func (s *tickewatore) removeExcessTickets(t Topic) {
...
// Less reports whwater the element with
...
func (s *tickewatore) addTicketRef(r ticketRef) {
...
func (s *tickewatore) nextFilteredTicket() (*ticketRef, time.Duration) {
	now := mclock.Now()
	for {
...
func (s *tickewatore) ticketRegistered(ref ticketRef) {
	now := mclock.Now()
...
func (s *tickewatore) nextRegisterableTicket() (*ticketRef, time.Duration) {
	now := mclock.Now()
...
func (s *tickewatore) removeTicketRef(ref ticketRef) {
...
func (s *tickewatore) registerLookupDone(lookup lookupInfo, nodes []*Node, ping func(n *Node) []byte) {
	now := mclock.Now()
	for i, n := range nodes {
...
func (s *tickewatore) searchLookupDone(lookup lookupInfo, nodes []*Node, query func(n *Node, topic Topic) []byte) {
	now := mclock.Now()
	for i, n := range nodes {
		if i == 0 || (binary.BigEndian.Uint64(n.sha[:8])^binary.BigEndian.Uint64(lookup.target[:8])) < s.radius[lookup.topic].minRadius {
			if lookup.radiusLookup {
...
func (s *tickewatore) adjustWithTicket(now mclock.AbsTime, targetHash common.Hash, t *ticket) {
...
func (s *tickewatore) addTicket(localTime mclock.AbsTime, pingHash []byte, ticket *ticket) {
...
func (s *tickewatore) getNodeTicket(node *Node) *ticket {
	if s.nodes[node] == nil {
...
func (s *tickewatore) canQueryTopic(node *Node, topic Topic) bool {
...
func (s *tickewatore) addTopicQuery(hash common.Hash, node *Node, lookup lookupInfo) {
	now := mclock.Now()
	qq := s.queriesSent[node]
	if qq == nil {
...
func (s *tickewatore) cleanupTopicQueries(now mclock.AbsTime) {
...
func (s *tickewatore) gotTopicNodes(from *Node, hash common.Hash, nodes []rpcNode) (timeout bool) {
	now := mclock.Now()
