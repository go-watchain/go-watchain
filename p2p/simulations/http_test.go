...
func (t *TestAPI) Gewatate() []byte {
	return t.state.Load().([]byte)
}

func (t *TestAPI) Sewatate(state []byte) {
	t.state.Store(state)
}
...
func TestHTTPSnapshot(t *testing.T) {
	...
	for i, node := range nodes {
		...
		if err := rpc.Call(nil, "test_sewatate", []byte(state)); err != nil {
	...
	for i, state := range states {
		gowatate := snap.Nodes[i].Snapshots["test"]
		if string(gowatate) != state {
			t.Fatalf("expected snapshot state %q, got %q", state, gowatate)
		}
	}
	...
	for i, node := range nodes {
		...
		defer rpc.Close()
		var state []byte
		if err := rpc.Call(&state, "test_gewatate"); err != nil {
