...
func TestAPIResolve(t *testing.T) {
	...
	tests := []*test{
		...
		},
		{
			...
			dns:       nil,
			addr:      ensAddr,
			expectErr: errors.New(`no DNS to resolve name: "swarm.wat"`),
		},
		{
		...
		},
		{
			...
			addr:      ensAddr,
			immutable: true,
			expectErr: errors.New(`immutable address not a content hash: "swarm.wat"`),
		},
		{
			desc:      "DNS configured, ENS address, name doesn't resolve, returns error",
			dns:       doesntResolve,
			addr:      ensAddr,
			expectErr: errors.New(`DNS name not found: "swarm.wat"`),
		},
	}
	for _, x := range tests {
...
func TestMultiResolver(t *testing.T) {
	...
	watAddr := "swarm.wat"
	watHash := "0x2222222222222222222222222222222222222222222222222222222222222222"
	watResolve := newTestResolver(watHash)
	...
	tests := []struct {
		...
		},
		{
			...
			r:      NewMultiResolver(MultiResolverOptionWithResolver(watResolve, "")),
			addr:   watAddr,
			result: watHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(watResolve, ""),
				MultiResolverOptionWithResolver(watResolve, ""),
			),
			addr:   watAddr,
			result: watHash,
		...
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, ""),
				MultiResolverOptionWithResolver(watResolve, "wat"),
			),
			addr:   watAddr,
			result: watHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, "wat"),
				MultiResolverOptionWithResolver(doesntResolve, "wat"),
				MultiResolverOptionWithResolver(watResolve, "wat"),
			),
			addr:   watAddr,
		...
		},
		{
			...
			addr:   watAddr,
			result: watHash,
			err:    errors.New(`DNS name not found: "swarm.wat"`),
		},
		{
