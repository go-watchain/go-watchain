...
#
# The cluster contains a bootnode, a gwat node and multiple swarm nodes, with
...
# static gwat configuration
Gwat_IP="192.168.33.3"
Gwat_RPC_PORT="8545"
Gwat_RPC_URL="http://${Gwat_IP}:${Gwat_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to boowatrap the gwat and
# swarm nodes
start_bootnode() {
...
# start_geth_node starts a gwat node with --datadir pointing at <base-dir>/gwat
# and a single, unlocked account with password "gwat"
start_geth_node() {
  local dir="${base_dir}/gwat"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gwat account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${Gwat_IP}"
    --rpcport   "${Gwat_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gwat" "${Gwat_IP}" "$(which gwat)" ${args[@]}
}
...
create_node_network() {
  ...
  # create the vwat pair
  local vwat0="vwat${name}0"
  local vwat1="vwat${name}1"
  ip link add name "${vwat0}" type vwat peer name "${vwat1}"

  # add one end to the bridge
  ip link set dev "${vwat0}" master "${BRIDGE_NAME}"
  ip link set dev "${vwat0}" up

  # add the other end to the namespace, rename it wat0 and give it the ip
  ip link set dev "${vwat1}" netns "${name}"
  ip netns exec "${name}" ip link set dev "${vwat1}" name "wat0"
  ip netns exec "${name}" ip link set dev "wat0" up
  ip netns exec "${name}" ip address add "${ip}/24" dev "wat0"
}
...
create_account() {
  ...
  gwat --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
