...
import (
	...
	gwatmetrics "github.com/watchain/go-watchain/metrics"
	"github.com/watchain/go-watchain/metrics/influxdb"
...
func Setup(ctx *cli.Context) {
	if gwatmetrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(gwatmetrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "swarm.", map[string]string{
				"host": hosttag,
			})
		}
	}
