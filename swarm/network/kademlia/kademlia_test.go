...
var (
	...
	quickcfgBoowatrap   = &quick.Config{MaxCount: 100, Rand: quickrand}
)
...
func TestBoowatrap(t *testing.T) {

	test := func(test *boowatrapTest) bool {
	...
	if err := quick.Check(test, quickcfgBoowatrap); err != nil {
		t.Error(err)
	}
...
type boowatrapTest struct {
	MaxProx    int
	BucketSize int
	Self       Address
}

func (*boowatrapTest) Generate(rand *rand.Rand, size int) reflect.Value {
	t := &boowatrapTest{
