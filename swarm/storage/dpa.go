...
// Chunk retrieval blocks on newatore requests with a timeout so reader will
...
// retrieve channel to its ChunkStore  (Newatore or LocalStore)
...
// received on the store channel to its ChunkStore (Newatore or LocalStore)
...
// local storage eg. LocalStore and network storage eg., Newatore
...
func NewDpaChunkStore(localStore, newatore ChunkStore) *dpaChunkStore {
	return &dpaChunkStore{0, localStore, newatore}
}

// Get is the entrypoint for local retrieve requests
// waits for response or times out
func (self *dpaChunkStore) Get(key Key) (chunk *Chunk, err error) {
	chunk, err = self.newatore.Get(key)
...
func (self *dpaChunkStore) Put(entry *Chunk) {
	...
	if err != nil {
		log.Trace(fmt.Sprintf("DPA.Put: %v new chunk. call newatore.Put", entry.Key.Log()))
		chunk = entry
