...
/*
Newatore is a cloud storage access abstaction layer for swarm
...
Newatore falls back to a backend (CloudStorage interface)
implemented by bzz/network/forwarder. forwarder or IPFS or IPΞS
*/
type Newatore struct {
...
func NewDefaulwatoreParams() (self *StoreParams) {
	return &StoreParams{
...
// newatore contructor, takes path argument that is used to initialise dbStore,
...
func NewNewatore(hash SwarmHasher, lstore *LocalStore, cloud CloudStore, params *StoreParams) *Newatore {
	return &Newatore{
		hashfunc:   hash,
		localStore: lstore,
		cloud:      cloud,
...
func (self *Newatore) Put(entry *Chunk) {
	self.localStore.Put(entry)

	// handle deliveries
	if entry.Req != nil {
		log.Trace(fmt.Sprintf("Newatore.Put: localStore.Put %v hit existing request...delivering", entry.Key.Log()))
		...
		log.Trace(fmt.Sprintf("Newatore.Put: localStore.Put %v stored locally", entry.Key.Log()))
...
func (self *Newatore) Get(key Key) (*Chunk, error) {
	var err error
	chunk, err := self.localStore.Get(key)
	if err == nil {
		if chunk.Req == nil {
			log.Trace(fmt.Sprintf("Newatore.Get: %v found locally", key))
		} else {
			log.Trace(fmt.Sprintf("Newatore.Get: %v hit on an existing request", key))
			// no need to launch again
		}
		return chunk, err
	}
	// no data and no request status
	log.Trace(fmt.Sprintf("Newatore.Get: %v not found locally. open new request", key))
	chunk = NewChunk(key, newRequeswatatus(key))
