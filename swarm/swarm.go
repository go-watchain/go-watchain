...
import (
	...
	"github.com/watchain/go-watchain/contracts/chequebook"
	"github.com/watchain/go-watchain/contracts/ens"
	...
	httpapi "github.com/watchain/go-watchain/swarm/api/http"
	"github.com/watchain/go-watchain/swarm/fuse"
	"github.com/watchain/go-watchain/swarm/network"
	"github.com/watchain/go-watchain/swarm/storage"
)
...
func NewSwarm(ctx *node.ServiceContext, backend chequebook.Backend, config *api.Config) (self *Swarm, err error) {
	...
	self.storage = storage.NewNewatore(hash, self.lstore, self.cloud, config.StoreParams)
...
func newEnsClient(endpoint string, addr common.Address, config *api.Config) (*ens.ENS, error) {
	...
	ensClient := watclient.NewClient(client)
...
func detectEnsAddr(client *rpc.Client) (common.Address, error) {
	...
	block, err := watclient.NewClient(client).BlockByNumber(ctx, big.NewInt(0))
	if err != nil {
		return common.Address{}, err
	}
