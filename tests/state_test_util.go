...
// See https://github.com/watereum/EIPs/issues/176 for the test format specification.
...
type stJSON struct {
	...
	Post map[string][]stPoswatate `json:"post"`
}

type stPoswatate struct {
...
func MakePreState(db watdb.Database, accounts core.GenesisAlloc) *state.StateDB {
...
func (tx *stTransaction) toMessage(ps stPoswatate) (core.Message, error) {
	...
	// Value, Data hex encoding is messy: https://github.com/watereum/tests/issues/203
	value := new(big.Int)
	if valueHex != "0x" {
