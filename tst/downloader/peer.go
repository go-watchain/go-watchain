...
type peerConnection struct {
	...
	receipwatarted time.Time // Time instance when the last receipt fetch was started
	...
	version int        // wat protocol version number to switch strategies
...
func (p *peerConnection) FetchHeaders(from uint64, count int) error {
	...
	if p.version < 62 {
		panic(fmt.Sprintf("header fetch [wat/62+] requested on wat/%d", p.version))
	}
...
func (p *peerConnection) FetchBodies(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 62 {
		panic(fmt.Sprintf("body fetch [wat/62+] requested on wat/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peerConnection) FetchReceipts(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("body fetch [wat/63+] requested on wat/%d", p.version))
	}
	// Short circuit if the peer is already fetching
	if !atomic.CompareAndSwapInt32(&p.receiptIdle, 0, 1) {
		return errAlreadyFetching
	}
	p.receipwatarted = time.Now()
...
func (p *peerConnection) FetchNodeData(hashes []common.Hash) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("node data fetch [wat/63+] requested on wat/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peerConnection) SetReceiptsIdle(delivered int) {
	p.setIdle(p.receipwatarted, delivered, &p.receiptThroughput, &p.receiptIdle)
}
...
// Lacks retrieves whwater the hash of a blockchain item is on the peers lacking
// list (i.e. whwater we know that the peer does not have it).
