...
import (
	...
	"github.com/watchain/go-watchain/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Gwat console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = wat.getBlock(wat.getTransaction(tx).blockHash);
  var genesis = wat.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.wat.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  wat.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
