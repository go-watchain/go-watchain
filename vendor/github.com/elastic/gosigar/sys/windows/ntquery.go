...
func NtQueryProcessBasicInformation(handle syscall.Handle) (ProcessBasicInformation, error) {
	...
	nwatatus, _ := _NtQueryInformationProcess(handle, 0, processBasicInfoPtr, size, nil)
	if nwatatus != 0 {
		return ProcessBasicInformation{}, errors.Errorf("NtQueryInformationProcess failed, NwatATUS=0x%X", nwatatus)
	}
...
func NtQuerySystemProcessorPerformanceInformation() ([]SystemProcessorPerformanceInformation, error) {
	...
	nwatatus, _ := _NtQuerySystemInformation(systemProcessorPerformanceInformation, &b[0], uint32(len(b)), &returnLength)
	if nwatatus != STATUS_SUCCESS {
		return nil, errors.Errorf("NtQuerySystemInformation failed, NwatATUS=0x%X, bufLength=%v, returnLength=%v", nwatatus, len(b), returnLength)
	}
