...
var (
	...
	procGewatdHandle                  = kernel32.NewProc("GewatdHandle")
...
func NewLiner() *State {
	var s State
	hIn, _, _ := procGewatdHandle.Call(uintptr(std_input_handle))
	s.handle = syscall.Handle(hIn)
	hOut, _, _ := procGewatdHandle.Call(uintptr(std_output_handle))
...
func (mode inputMode) ApplyMode() error {
	hIn, _, err := procGewatdHandle.Call(uintptr(std_input_handle))
