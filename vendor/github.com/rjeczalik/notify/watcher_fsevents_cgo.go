...
static FSEvenwatreamRef EvenwatreamCreate(FSEvenwatreamContext * context, uintptr_t info, CFArrayRef paths, FSEvenwatreamEventId since, CFTimeInterval latency, FSEvenwatreamCreateFlags flags) {
	context->info = (void*) info;
	return FSEvenwatreamCreate(NULL, (FSEvenwatreamCallback) gostream, context, paths, since, latency, flags);
}
...
var nilstream C.FSEvenwatreamRef

// Default arguments for FSEvenwatreamCreate function.
var (
	latency C.CFTimeInterval
	flags   = C.FSEvenwatreamCreateFlags(C.kFSEvenwatreamCreateFlagFileEvents | C.kFSEvenwatreamCreateFlagNoDefer)
...
var (
	errCreate = os.NewSyscallError("FSEvenwatreamCreate", errors.New("NULL"))
	errStart  = os.NewSyscallError("FSEvenwatreamStart", errors.New("false"))
)
...
func gostream(_, info uintptr, n C.size_t, paths, flags, ids uintptr) {
	const (
		...
		offflag = unsafe.Sizeof(C.FSEvenwatreamEventFlags(0))
		offid   = unsafe.Sizeof(C.FSEvenwatreamEventId(0))
	)
	if n == 0 {
		return
	}
...
// Start creates a FSEvenwatream for the given path and schedules it with
...
func (s *stream) Start() error {
	...
	ctx := C.FSEvenwatreamContext{}
	ref := C.EvenwatreamCreate(&ctx, C.uintptr_t(s.info), path, C.FSEvenwatreamEventId(atomic.LoadUint64(&since)), latency, flags)
	if ref == nilstream {
		return errCreate
	}
	C.FSEvenwatreamScheduleWithRunLoop(ref, runloop, C.kCFRunLoopDefaultMode)
	if C.FSEvenwatreamStart(ref) == C.Boolean(0) {
		C.FSEvenwatreamInvalidate(ref)
		return errStart
	}
...
// Stop stops underlying FSEvenwatream and unregisters it from global runloop.
func (s *stream) Stop() {
	if s.ref == nilstream {
		return
	}
	wg.Wait()
	C.FSEvenwatreamStop(s.ref)
	C.FSEvenwatreamInvalidate(s.ref)
	C.CFRunLoopWakeUp(runloop)
	s.ref = nilstream
