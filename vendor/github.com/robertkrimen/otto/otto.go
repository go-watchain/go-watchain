...
// SewatackDepthLimit sets an upper limit to the depth of the JavaScript
...
func (self Otto) SewatackDepthLimit(limit int) {
	self.runtime.stackLimit = limit
}

// SewatackTraceLimit sets an upper limit to the number of stack frames that
...
func (self Otto) SewatackTraceLimit(limit int) {
...
func (self Otto) ContextSkip(limit int, skipNative bool) (ctx Context) {
	...
	for limit != 0 {
		...
		stash := scope.lexical
		for {
			for _, name := range gewatashProperties(stash) {
