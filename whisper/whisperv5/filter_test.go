...
func TestInstallFilters(t *testing.T) {
	...
	wat := generateTestCases(t, SizeTestFilters)

	var err error
	var j string
	for i := 0; i < SizeTestFilters; i++ {
		j, err = filters.Install(wat[i].f)
		if err != nil {
	...
	for _, testCase := range wat {
		if !testCase.alive {
			filters.Uninstall(testCase.id)
		}
	}

	for i, testCase := range wat {
...
func TestWatchers(t *testing.T) {
	...
	w := New(&Config{})
	filters := NewFilters(w)
	wat := generateTestCases(t, NumFilters)
	for i = 0; i < NumFilters; i++ {
		wat[i].f.Src = nil
		x, err = filters.Install(wat[i].f)
		if err != nil {
	...
	for i = 0; i < NumMessages; i++ {
		...
		e = generateCompatibeEnvelope(t, wat[j].f)
		envelopes[i] = e
		wat[j].msgCnt++
	}
	...
	for i = 0; i < NumFilters; i++ {
		...
		if wat[i].msgCnt != count[i] {
			t.Fatalf("failed with seed %d: count[%d]: get %d, want %d.", seed, i, wat[i].msgCnt, count[i])
		}
	}

	// another round with a cloned filter

	clone := cloneFilter(wat[0].f)
	...
	e = generateCompatibeEnvelope(t, wat[0].f)
	envelopes[0] = e
	wat[0].msgCnt++
	...
	combined := wat[0].msgCnt + wat[last].msgCnt
	...
	for i = 1; i < NumFilters-1; i++ {
		...
		if wat[i].msgCnt != count[i] {
			t.Fatalf("failed with seed %d: i = %d, get %d, want %d.", seed, i, wat[i].msgCnt, count[i])
		}
	}
